...
! SPHERE_MHD code
!
! Spectral code to evolve the magnetohydrodynamic equations in 
! a sphere using the Chandrasekhar-Kendall eigenfunctions of 
...
! conditions in the rotating frame are u.n=w.n=b.n=j.n=0 (no 
! velocity and vorticity perpendicular to the surface of the 
! sphere, perfect conductor outside the sphere for the magnetic 
! field).
!
...
! Tables and expansion coefficients
      ...
      DOUBLE COMPLEX, ALLOCATABLE, DIMENSION(:,:)         :: xsiv,xsib
      DOUBLE COMPLEX, ALLOCATABLE, DIMENSION(:,:)         :: xsi1,xss1
      DOUBLE COMPLEX, ALLOCATABLE, DIMENSION(:,:)         :: xsi2,xss2
...
! Some auxiliary matrices
      ...
      DOUBLE COMPLEX     :: lorent
      DOUBLE COMPLEX     :: induct
      DOUBLE COMPLEX     :: xv2,xv3
      DOUBLE COMPLEX     :: xb2,xb3
...
! Namelists for the input files
      NAMELIST / status / stat,bench,corio,angu,outs,edge,grid,precs,dyna
      NAMELIST / parameter / dt,step,gstep,sstep,bstep
      NAMELIST / parameter / f0,p0,u0,b0,nu,mu,rand,cort,seed,tdir,odir
...
!     dyna : if >0 sets the file number used as initial conditions 
!            for the velocity in a dynamo simulation
...
!
! Reads from the external file 'parameter.inp' the 
...
!     b0   : amplitude of the initial magnetic field
...
! Allocates memory
      ...
      ALLOCATE( xsi2(l*(l+3)/2,2*q), xss2(l*(l+3)/2,2*q) )
      ALLOCATE( xsiv(l*(l+3)/2,2*q), xsib(l*(l+3)/2,2*q) )
...
! case, a previous run is continued.
      ...
      IF (stat.eq.0) THEN
         ...
         times = sstep
         timeb = bstep
         INCLUDE 'initialb.f90'         ! initial conditions for b
         IF (dyna.eq.0) THEN            ! initial conditions for v
            INCLUDE 'initialv.f90'
         ELSE                           ! dynamo run
            IF (myrank.eq.0) THEN
               CALL genext(dyna,ext)
      ...
      ELSE
         ...
         IF (myrank.eq.0) THEN
            ...
            READ(1) xsiv
            CLOSE(1)
            OPEN(1,file=trim(odir) // '/xsib.' // ext // '.out', &
                 form='unformatted')
            READ(1) xsib
            CLOSE(1)
         ENDIF
         CALL MPI_BCAST(xsiv,(l+3)*l*q,MPI_DOUBLE_COMPLEX,0, &
              MPI_COMM_WORLD,ierr)
         CALL MPI_BCAST(xsib,(l+3)*l*q,MPI_DOUBLE_COMPLEX,0, &
              MPI_COMM_WORLD,ierr)
         bindex = bindex+1
...
! with global quantities
      ...
      IF (timeg.eq.gstep) THEN
         ...
         IF (myrank.eq.0) THEN
            ...
            CALL global((t-1)*dt,lambda,xsib,1)
            CALL injections((t-1)*dt,lambda,xsiv,normal,forv,wtx,wty,wtz)
            CALL xsi2ckp(xsiv,lambda,normal,edge,grid,48,48,48,0)
            CALL xsi2ckp(xsib,lambda,normal,edge,grid,48,48,48,1)
            !OPEN(1,file='kmode.1_1.txt',position='append')
            !   WRITE(1,*) REAL(xsiv(1,1)),AIMAG(xsiv(1,1))
            !CLOSE(1)
            !OPEN(1,file='kmode.2_1.txt',position='append')
            !   WRITE(1,*) REAL(xsiv(2,1)),AIMAG(xsiv(2,1))
            !CLOSE(1)
            !OPEN(1,file='kmode.2_2.txt',position='append')
            !   WRITE(1,*) REAL(xsiv(2,2)),AIMAG(xsiv(2,2))
            !CLOSE(1)
            !OPEN(1,file='kmode.12_4.txt',position='append')
            !   WRITE(1,*) REAL(xsiv(12,4)),AIMAG(xsiv(12,4))
            !CLOSE(1)
            !OPEN(1,file='kmode.14_4.txt',position='append')
            !   WRITE(1,*) REAL(xsiv(14,4)),AIMAG(xsiv(14,4))
            !CLOSE(1)
            !OPEN(1,file='mmode.1_1.txt',position='append')
            !   WRITE(1,*) REAL(xsib(1,1)),AIMAG(xsib(1,1))
            !CLOSE(1)
            !OPEN(1,file='mmode.2_1.txt',position='append')
            !   WRITE(1,*) REAL(xsib(2,1)),AIMAG(xsib(2,1))
            !CLOSE(1)
            !OPEN(1,file='mmode.2_2.txt',position='append')
            !   WRITE(1,*) REAL(xsib(2,2)),AIMAG(xsib(2,2))
            !CLOSE(1)
            !OPEN(1,file='mmode.12_4.txt',position='append')
            !   WRITE(1,*) REAL(xsib(12,4)),AIMAG(xsib(12,4))
            !CLOSE(1)
            !OPEN(1,file='mmode.14_4.txt',position='append')
            !   WRITE(1,*) REAL(xsib(14,4)),AIMAG(xsib(14,4))
            !CLOSE(1)
            IF (angu.eq.1) THEN
               CALL momentum(lambda,normal,xsiv,0)
               CALL momentum(lambda,normal,xsib,1)
            ENDIF
         ENDIF
      ENDIF
      timeg = timeg+1
...
! with the power spectrum
      ...
      IF (times.eq.sstep) THEN
         ...
         IF (myrank.eq.0) THEN
            ...
            CALL spectruml(lambda,xsib,ext,1)
            CALL spectrumq(lambda,xsiv,ext,0)
            CALL spectrumq(lambda,xsib,ext,1)
         ENDIF
         sindex = sindex+1
      ENDIF
      times = times+1
      ...
      IF (timeb.eq.bstep) THEN
         ...
         IF (myrank.eq.0) THEN
            ...
            IF (outs.eq.1) THEN
               ...
               CALL xsi2ck(xsib,lambda,normal,edge,grid,odir,ext,1)
            ENDIF
         ENDIF
         bindex = bindex+1
      ENDIF
...
! Runge-Kutta of order 'ord'
 ...
 OM :    IF (ABS(m3).le.l3) THEN        ! -l<=m<=l
            IF (m3.ge.0) THEN           ! xsi(-m3) = (-1)^m3*CONJG(xsi(m3))
               ...
               xb3 = xss2(l3*(l3+1)/2+m3,q3)
            ELSE
               xv3 = (-1)**ABS(m3)*CONJG(xss1(l3*(l3+1)/2-m3,q3))
               xb3 = (-1)**ABS(m3)*CONJG(xss2(l3*(l3+1)/2-m3,q3))
            ENDIF
            IF (q3.le.q) THEN
               lam3 = lambda(l3,q3)
            ELSE
            ...
            lorent = lorent+lam2*cicoef(l3,ind2,q3,q2,ind1,q1)*xb2*xb3
            induct = induct+lam1*cicoef(l3,ind2,q3,q2,ind1,q1)*xv2*xb3
         ENDIF OM
         END DO
         END DO
         END DO
 ...
 PR :    IF (precs.eq.1) THEN
         ...
         ENDIF
         xsi1(ind1,q1) = xsiv(ind1,q1)+dt*(nonlin+coriolis+lorent+precession- &
                         nu*lam1**2*xss1(ind1,q1)+forv(ind1,q1))/o
         xsi2(ind1,q1) = xsib(ind1,q1)+dt*(induct-mu*lam1**2*xss2(ind1,q1))/o

      ENDDO
      ENDDO
 SY : DO irank = 0,nprocs-1             ! synchronization
         CALL MPI_BCAST(xsi1,1,xtype(irank),irank,MPI_COMM_WORLD,ierr)
         CALL MPI_BCAST(xsi2,1,xtype(irank),irank,MPI_COMM_WORLD,ierr)
      ENDDO SY
      ENDDO RD
      xsiv = xsi1
      xsib = xsi2
